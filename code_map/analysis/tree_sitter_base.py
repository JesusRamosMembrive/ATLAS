...
"""
TreeSitterMixin: Shared tree-sitter operations for all code analyzers.

Provides unified tree-sitter initialization and common tree traversal utilities.
This mixin eliminates code duplication across:
- Call Flow extractors (Python, TypeScript, C++)
- UML analyzers (TypeScript, C++)

Usage:
    class MyAnalyzer(TreeSitterMixin):
...
class TreeSitterMixin:
    ...
    All code analyzers that use tree-sitter can inherit from this mixin to share:
    - Parser initialization with consistent error handling
    - Common tree traversal methods
    - Node text extraction
    - Child/descendant lookup utilities

    Attributes:
        LANGUAGE: Tree-sitter language name (override in subclass)
        _parser: Cached tree-sitter parser instance
    ...
    def _find_all_descendants_by_type(self, node: Any, type_name: str) -> List[Any]:
        """
        Find all descendants of given type (recursive search).
        ...
        Returns:
            List of all descendants matching the type.
        """
        return [d for d in self._walk_tree(node) if d.type == type_name]

    def _get_identifier_text(self, node: Any, source: bytes) -> Optional[str]:
        """
        Extract identifier text from a node that contains an identifier child.

        Commonly used for function names, class names, variable names, etc.

        Args:
            node: Node that may contain an identifier.
            source: Source code as bytes.
        ...
        if identifier:
            return self._get_node_text(identifier, source)
        return None
